...
public class MessageAdapter  extends RecyclerView.Adapter<MessageAdapter.MessageHolder> {
    ...
    public void onBindViewHolder(@NonNull MessageHolder holder, int position) {
        ...
        if (messages.get(position).getSender().equals(FirebaseAuth.getInstance().getCurrentUser().getEmail())) {
            ...
            Glide.with(context).load(senderImg).error(R.drawable.baseline_person_24).placeholder(R.drawable.baseline_person_24).into(holder.profImage);

            ConstraintSet constraintSet = new ConstraintSet();
            constraintSet.clone(constraintLayout);
            constraintSet.clear(R.id.profile_cardview, ConstraintSet.LEFT);
            constraintSet.clear(R.id.txt_msg_content, ConstraintSet.LEFT);
            constraintSet.connect(R.id.profile_cardview,ConstraintSet.RIGHT, R.id.ccLayout,ConstraintSet.RIGHT,0);
            constraintSet.connect(R.id.txt_msg_content,ConstraintSet.RIGHT, R.id.profile_cardview,ConstraintSet.LEFT,0);
            constraintSet.applyTo(constraintLayout);

            
        }else {

            Glide.with(context).load(receiverImg).error(R.drawable.baseline_person_24).placeholder(R.drawable.baseline_person_24).into(holder.profImage);

            ConstraintSet constraintSet = new ConstraintSet();
            constraintSet.clone(constraintLayout);
            constraintSet.clear(R.id.profile_cardview, ConstraintSet.RIGHT);
            constraintSet.clear(R.id.txt_msg_content, ConstraintSet.RIGHT);
            constraintSet.connect(R.id.profile_cardview,ConstraintSet.LEFT, R.id.ccLayout,ConstraintSet.LEFT,0);
            constraintSet.connect(R.id.txt_msg_content,ConstraintSet.LEFT, R.id.profile_cardview,ConstraintSet.RIGHT,0);
            constraintSet.applyTo(constraintLayout);
    ...
    class MessageHolder  extends RecyclerView.ViewHolder{
        ...
        public MessageHolder(@NonNull View itemView) {
            ...
            txtMessage = itemView.findViewById(R.id.txt_msg_content);
